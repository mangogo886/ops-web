...
-- 卡口审核抽检记录表
-- ============================================
-- 说明：用于记录卡口审核任务的抽检历史，支持多次抽检
-- 执行时间：2025-12-20
-- 功能：保存每次抽检的详细信息，包括抽检人员、时间、意见、结果等

-- 检查 checkpoint_tasks 表是否存在（依赖表）
SET @table_exists := (
    SELECT COUNT(*) 
...
SET @sqlstmt := IF(
    ...
    'SELECT "checkpoint_tasks 表存在，开始创建抽检记录表" AS message'
);
...
SET @sample_table_exists := (
    ...
    FROM information_schema.TABLES 
    WHERE TABLE_SCHEMA = DATABASE() 
    AND TABLE_NAME = 'checkpoint_sample_records'
);

-- 如果表不存在，则创建表
-- 注意：CREATE TABLE IF NOT EXISTS 语句不能放在动态 SQL 中，直接执行
CREATE TABLE IF NOT EXISTS `checkpoint_sample_records` (
  ...
  `sample_result` VARCHAR(20) DEFAULT NULL COMMENT '抽检结果：通过、待整改',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='卡口审核抽检记录表';

-- 完成提示
SELECT "卡口审核抽检记录表创建完成" AS message;
