...
-- 设备审核任务表添加抽检功能字段
-- ============================================
-- 说明：为 audit_tasks 表添加抽检相关字段，用于快速查询和显示抽检状态
-- 执行时间：2025-12-19
...
SET @sqlstmt := IF(
    ...
    'SELECT "audit_tasks 表存在，开始添加抽检字段" AS message'
);
...
SET @sqlstmt2 := IF(
    @is_sampled_exists = 0,
    'ALTER TABLE `audit_tasks` ADD COLUMN `is_sampled` TINYINT(1) NOT NULL DEFAULT 0 COMMENT ''是否已抽检：0-未抽检，1-已抽检'' AFTER `audit_status`',
...
SET @sqlstmt3 := IF(
    ...
    'ALTER TABLE `audit_tasks` ADD COLUMN `last_sampled_at` TIMESTAMP NULL DEFAULT NULL COMMENT ''最后抽检时间'' AFTER `is_sampled`',
...
SET @sqlstmt4 := IF(
    @index_exists = 0,
    'ALTER TABLE `audit_tasks` ADD INDEX `idx_is_sampled` (`is_sampled`)',
...
-- 完成提示
SELECT "设备审核任务表抽检字段添加完成" AS message;
