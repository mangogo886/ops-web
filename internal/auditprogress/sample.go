...
func GetSampleInfo(taskID int) (*SampleInfo, error) {
	...
	// 如果有最后抽检时间，查询最后抽检的人员信息
	if lastSampledAt.Valid && lastSampledAt.String != "" {
		info.LastSampledAt = formatDateTime(lastSampledAt.String)
		
		// 查询最后一条抽检记录获取抽检人员
		var sampledBy sql.NullString
		lastRecordSQL := `SELECT sampled_by FROM audit_sample_records 
			WHERE task_id = ? 
			ORDER BY sampled_at DESC 
			LIMIT 1`
		err = db.DBInstance.QueryRow(lastRecordSQL, taskID).Scan(&sampledBy)
		if err == nil && sampledBy.Valid {
...
func BatchGetSampleInfo(taskIDs []int) (map[int]*SampleInfo, error) {
	...
	if len(result) > 0 {
		...
		for taskID := range result {
			var sampledBy sql.NullString
			var count int
			lastSQL := `SELECT sampled_by, 
				(SELECT COUNT(*) FROM audit_sample_records WHERE task_id = ?) AS sample_count
				FROM audit_sample_records 
				WHERE task_id = ? 
				ORDER BY sampled_at DESC 
				LIMIT 1`
			err = db.DBInstance.QueryRow(lastSQL, taskID, taskID).Scan(&sampledBy, &count)
			if err == nil {
				if sampledBy.Valid {
