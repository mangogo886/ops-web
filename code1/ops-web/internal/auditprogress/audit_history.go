...
func SaveAuditHistory(tx *sql.Tx, taskID int, newComment string, newStatus string, currentUser *auth.User) error {
	...
	selectSQL := `SELECT audit_comment, audit_status FROM audit_tasks WHERE id = ?`
	...
	if needSaveHistory {
		...
		// 插入历史记录
		insertHistorySQL := `INSERT INTO audit_audit_history (task_id, audit_comment, audit_status, auditor, audit_time) VALUES (?, ?, ?, ?, NOW())`
...
func GetAuditHistory(taskID int) ([]HistoryItem, error) {
	historySQL := `SELECT id, audit_comment, audit_status, auditor, audit_time 
		FROM audit_audit_history 
		WHERE task_id = ? 
		ORDER BY audit_time DESC`

	rows, err := db.DBInstance.Query(historySQL, taskID)
	if err != nil {
		logger.Errorf("查询审核意见历史失败: %v, taskID: %d", err, taskID)
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err != nil {
			logger.Errorf("扫描审核意见历史记录失败: %v", err)
			continue
		}
	...
	if err = rows.Err(); err != nil {
		logger.Errorf("遍历审核意见历史记录失败: %v", err)
		return nil, err
	}
